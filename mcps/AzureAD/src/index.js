...
class AzureADMCPServer {
  constructor() {
    ...
    this.scopes = (
      process.env.AZURE_SCOPES || "https://graph.microsoft.com/.default"
    ).split(",")

    if (!this.msalConfig.auth.clientId) {
      console.error(
        "Warning: AZURE_CLIENT_ID not set. Authentication will fail."
      )
    }
    ...
    try {
      if (fs.existsSync(this.tokenCachePath)) {
        ...
        if (
          cache.accessToken &&
          cache.expiry &&
          new Date(cache.expiry) > new Date()
        ) {
  ...
  async handleCheckAuthStatus() {
    ...
    const status = isAuthenticated
      ? ` Authenticated\n\nToken expires: ${this.tokenExpiry.toLocaleString()}\nTime remaining: ${Math.floor(
          (this.tokenExpiry - new Date()) / 1000 / 60
        )} minutes`
    ...
    return {
      content: [
        {
          type: "text",
          text: `•••••••••••• MCP.AzureAD: check_auth_status ••••••••••••\n${status}`,
        },
      ],
    }
  }
  ...
  async handleClearTokenCache() {
    ...
    return {
      content: [
        {
          type: "text",
          text: "•••••••••••• MCP.AzureAD: clear_token_cache ••••••••••••\n Token cache cleared",
        },
      ],
    }
  }
    ...
    try {
      ...
      return {
        content: [
          {
            type: "text",
            text: `•••••••••••• MCP.AzureAD: make_authenticated_request ••••••••••••\nStatus: ${
              response.status
            } ${response.statusText}\n\nResponse:\n\n${JSON.stringify(
              response.data,
              null,
