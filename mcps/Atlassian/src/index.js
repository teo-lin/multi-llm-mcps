...
class AtlassianServer {
  ...
  async handleTicketInfo(args) {
    ...
    return {
      content: [
        {
          type: "text",
          text: `•••••••••••• MCP.Atlassian: jira_ticket_info ••••••••••••\n${JSON.stringify(
            ticket,
            null,
            2
          )}`,
  ...
  async handleExtractTicket(args) {
    ...
    return {
      content: [
        {
          type: "text",
          text: `•••••••••••• MCP.Atlassian: jira_extract_ticket_from_text ••••••••••••\n${JSON.stringify(
            { ticketKey },
            null,
            2
          )}`,
  ...
  async handleOpenTicket(args) {
    ...
    return {
      content: [
        {
          type: "text",
          text: `•••••••••••• MCP.Atlassian: jira_open_ticket ••••••••••••\n${JSON.stringify(
            { opened: true, ticket: args.ticket_key },
            null,
            2
          )}`,
        },
  ...
  async handleAuthStatus() {
    ...
    return {
      content: [
        {
          type: "text",
          text: `•••••••••••• MCP.Atlassian: jira_auth_status ••••••••••••\n${JSON.stringify(
  ...
  async _getTicketInfoViaBasicAuth(ticketKey) {
    ...
    return {
      ...
      summary: ticket.fields.summary,
      description:
        this.extractDescriptionText(ticket.fields.description) ||
        "No description",
      acceptanceCriteria: this.extractAcceptanceCriteria(
        ticket.fields.description
      ),
  ...
  async getJiraTicketInfo(ticketKey) {
    ...
    if (JIRA_AUTH_STRATEGY === "oauth" || JIRA_AUTH_STRATEGY === "auto") {
      ...
      if (oauthAvailable) {
        try {
          log.info(
            `[Atlassian MCP] Fetching ticket ${ticketKey} via OAuth (acli)`
          )
    ...
    if (JIRA_AUTH_STRATEGY === "basic" || JIRA_AUTH_STRATEGY === "auto") {
      try {
        log.info(
          `[Atlassian MCP] Fetching ticket ${ticketKey} via Basic Auth (API token)`
        )
  ...
  async attemptAutoAuth() {
    try {
      ...
      try {
        await execAsync(
          `acli jira auth login --site "${JIRA_SITE}" --email "${JIRA_EMAIL}" --token < ${tokenFile}`
        )
