...
class MSKMCPServer {
  ...
  async handleGetClusterDetails(args) {
    ...
    return {
      content: [
        {
          type: "text",
          text: `•••••••••••• MCP.MSK: get_cluster_details ••••••••••••\n${JSON.stringify(
            {
              status: "success",
              cluster: details,
  ...
  async handleGetTopicMetadata(args) {
    ...
    const brokers = await this.mskClient.getBootstrapBrokers(clusterArn)
    const metadata = await this.kafkaManager.getTopicMetadata(
      brokers,
      clusterArn,
      topicName
    )
    ...
    return {
      content: [
        {
          type: "text",
          text: `•••••••••••• MCP.MSK: get_topic_metadata ••••••••••••\n${JSON.stringify(
            {
              status: "success",
              cluster: clusterArn,
              topic: metadata,
  ...
  async handleBrowseMessages(args) {
    ...
    if (decodeProtobuf && !messageType) {
      throw new Error(
        "messageType is required when decodeProtobuf is true. Use list_protobuf_types to see available types."
      )
    }
    ...
    const messages = rawMessages.map((msg) => {
      ...
      if (decodeProtobuf && messageType && base64Value) {
        const decodeResult = this.protobufDecoder.tryDecode(
          base64Value,
          messageType
        )
  ...
  async handleListProtobufTypes(args) {
    ...
    return {
      content: [
        {
          type: "text",
          text: `•••••••••••• MCP.MSK: list_protobuf_types ••••••••••••\n${JSON.stringify(
            {
              status: "success",
              count: types.length,
              types,
