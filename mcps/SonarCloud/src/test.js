  ...
  lines.forEach((line) => {
    ...
    try {
      ...
      if (response.id === 1 && response.result?.capabilities) {
        console.log(
          " Test 1: Initialize - Server initialized with capabilities"
        )
      ...
      if (response.id === 3) {
        if (
          ...
          response.result?.content?.[0]?.text?.includes("not configured")
        ) {
          console.log(
            "  Test 3: Get Project Status - Expected auth error (no credentials configured)"
          )
      ...
      if (response.id === 4) {
        if (
          ...
          console.log(
            "  Test 4: Get Measures - Expected auth error (no credentials configured)"
          )
...
mcp.stderr.on("data", (data) => {
  ...
  if (msg.includes("Warning: SonarCloud not configured")) {
    console.log(
      "ℹ  Info: SonarCloud credentials not configured (expected for testing)"
    )
  }
})
