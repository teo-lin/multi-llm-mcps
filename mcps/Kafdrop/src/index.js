...
class KafdropMCPServer {
  ...
  async kafdropRequest(endpoint) {
    try {
      ...
      if (error.response) {
        throw new Error(
          `Kafdrop API error: ${error.response.status} - ${error.response.statusText}`
        )
      } else if (error.request) {
        throw new Error(
          `Cannot connect to Kafdrop at ${this.kafdropUrl}. Is it running?`
        )
      } else {
  ...
  async getTopicDetails(args) {
    ...
    const data = await this.kafdropRequest(
      `/topic/${encodeURIComponent(topic_name)}`
    )
    ...
    return {
      content: [
        {
          type: "text",
          text: `•••••••••••• MCP.Kafdrop: get_topic_details ••••••••••••\nTopic: ${topic_name}\n\n${JSON.stringify(
            data,
            null,
            2
          )}`,
  ...
  async browseMessages(args) {
    ...
    let endpoint = `/topic/${encodeURIComponent(
      topic_name
    )}/${partition}/messages`
  ...
  async listConsumerGroups() {
    ...
    return {
      content: [
        {
          type: "text",
          text: `•••••••••••• MCP.Kafdrop: list_consumer_groups ••••••••••••\nFound ${
            groups.length
  ...
  async getConsumerGroupDetails(args) {
    const { group_id } = args
    const data = await this.kafdropRequest(
      `/consumer/${encodeURIComponent(group_id)}`
    )
    ...
    return {
      content: [
        {
          type: "text",
          text: `•••••••••••• MCP.Kafdrop: get_consumer_group_details ••••••••••••\nConsumer Group: ${group_id}\n\n${JSON.stringify(
            data,
            null,
            2
          )}`,
  ...
  async searchMessages(args) {
    ...
    const topicData = await this.kafdropRequest(
      `/topic/${encodeURIComponent(topic_name)}`
    )
    const partitions =
      partition !== undefined
        ? [partition]
        : (topicData.partitions || []).map((p, i) =>
            p.id !== undefined ? p.id : i
          )
  ...
  async run() {
    ...
    console.error(
      `Kafdrop MCP Server running on stdio (connecting to ${this.kafdropUrl})`
    )
  }
}
