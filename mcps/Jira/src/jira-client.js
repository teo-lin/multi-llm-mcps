...
export class JiraClient {
  ...
  async _getTicketViaOAuth(ticketKey) {
    const { stdout } = await execAsync(`acli jira workitem view ${ticketKey} --json`);
  ...
  async getTicketDetails(ticketKey) {
    ...
    if (this.authStrategy === 'oauth' || this.authStrategy === 'auto') {
      ...
      if (oauthAvailable) {
        try {
          console.error(`[Jira MCP] Fetching ticket ${ticketKey} via OAuth (acli)`);
          ...
          if (this.authStrategy === 'oauth') {
            throw new Error(`Failed to fetch Jira ticket ${ticketKey} via OAuth: ${error.message}`);
          }
        }
      }
    }
    ...
    if (this.authStrategy === 'basic' || this.authStrategy === 'auto') {
      try {
        console.error(`[Jira MCP] Fetching ticket ${ticketKey} via Basic Auth (API token)`);
    ...
    throw new Error(`Failed to fetch Jira ticket ${ticketKey}. All auth methods failed: ${errors.join(', ')}`);
  }

  async _searchTicketsViaOAuth(jql, maxResults) {
    const { stdout } = await execAsync(
      `acli jira workitem search --jql "${jql.replace(/"/g, '\\"')}" --limit ${maxResults} --json`
    );
  ...
  async searchTickets(jql, maxResults = 50) {
    ...
    if (this.authStrategy === 'oauth' || this.authStrategy === 'auto') {
      ...
      if (oauthAvailable) {
        try {
          ...
          if (this.authStrategy === 'oauth') {
            throw new Error(`Failed to search Jira tickets via OAuth: ${error.message}`);
          }
        }
      }
    }
    ...
    if (this.authStrategy === 'basic' || this.authStrategy === 'auto') {
      try {
        console.error(`[Jira MCP] Searching tickets via Basic Auth (API token)`);
    ...
    throw new Error(`Failed to search Jira tickets. All auth methods failed: ${errors.join(', ')}`);
  }
  ...
  async getBoardIssues(boardId, sprintId) {
    ...
    if (this.authStrategy === 'basic' || (this.authStrategy === 'auto' && !await this._isOAuthAvailable())) {
      if (!this.hasBasicAuth) {
      ...
      try {
        ...
        console.error(`[Jira MCP] Failed to get board issues via basic auth: ${error.message}`);
      }
    }
  ...
  async getTeamNames() {
    ...
    if (this.hasBasicAuth) {
      try {
        const response = await this.axiosClient.get("/rest/api/3/field/search", {
          params: {
            query: "team",
            type: "custom",
          },
        ...
        console.warn("Could not fetch team names via API, returning defaults:", error);
      }
    }
