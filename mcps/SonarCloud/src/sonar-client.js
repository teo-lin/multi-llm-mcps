...
export class SonarClient {
  constructor() {
    ...
    if (!this.configured) {
      console.error(
        "  Warning: SonarCloud not configured. Set SONAR_TOKEN, SONAR_ORGANIZATION, and SONAR_PROJECT_KEY environment variables."
      )
      this.client = null
      return
    }
  ...
  async getIssues(options = {}) {
    ...
    if (severities)
      params.severities = Array.isArray(severities)
        ? severities.join(",")
        : severities
  ...
  async getMeasures(
    ...
    const response = await this.client.get("/api/measures/component", {
      params: {
        ...
        metricKeys: Array.isArray(metricKeys)
          ? metricKeys.join(",")
          : metricKeys,
      },
    })
  ...
  async getRules(options = {}) {
    ...
    if (languages)
      params.languages = Array.isArray(languages)
        ? languages.join(",")
        : languages
    if (types) params.types = Array.isArray(types) ? types.join(",") : types
    if (severities)
